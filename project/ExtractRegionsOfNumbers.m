function [ Results ] = ExtractRegionsOfNumbers(I)
...
for i=1:h
    ...
    if(~(MainHieght > 30 && MainHieght < 100 && MainWidth > 30 && MainWidth < 110 &&  bounds(i).Extent >.3 && bounds(i).Area > 500 && (max(MainHieght,MainWidth)/min(MainHieght,MainWidth))>=1 && (max(MainHieght,MainWidth)/min(MainHieght,MainWidth))<=3 ))
        continue;
    end
    for j=1 : h
        ...
        if(i == j || ~(bounds(j).Extent >.3 && bounds(j).Area > 500 && NearHieght > 30 && NearHieght < 100 && NearWidth > 30 && NearWidth < 110 )) 
            continue;
        end
